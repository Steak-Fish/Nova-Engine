...
Device::Device(Window &window, EngineConfig c) {

  createInstance();
  setupDebugMessenger();
  createSurface(window);
  pickPhysicalDevice(c);
  createLogicalDevice();
  createCommandPool();
}
...
void Device::pickPhysicalDevice(EngineConfig c) {
  ...
  uint32_t ForceGPUID = c.forceGPUID;
  if (c.forceGPU) {
    std::cout << "Forced GPU ID: " << ForceGPUID << "\n";
    for (const auto &device : devices) {
      vkGetPhysicalDeviceProperties(device, &properties);
...
void Device::createSurface(Window& window) { window.createWindowSurface(instance, &surface_); }
